...
void main(List<String> arguments) {
  ...
  .listen((String line) {
      ...
      List<int> phases = [5,6,7,8,9];
      var combos = Permutations(5, phases);
      for (var combo in combos()) {
        int input = 0;
        int ampId = 0;
        List<List<int>> programs = [];
        List<int> iStore = [];
        while (true) {
          var args = [input];
          // one-time setup (this check is ugly, why cant i say if(programs[ampId] == null?)
          if(programs.isEmpty || programs.length < ampId+1) {
            programs.add([...inputs]);
            iStore.add(0);
            args.insert(0, combo[ampId]);
          }
          var output = handle(programs[ampId], args, iStore[ampId]);
          if(output == null) break;
          input = output['output'];
          iStore[ampId] = output['iJump'];
          ampId = ampId < 4 ? ampId+1 : 0;
        }
        outputs.add(input);
      }
    },
...
Map<String,int> handle(List<int> nums, List<int> inputs, [int iJump = 0]) {
